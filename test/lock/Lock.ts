  ...
  describe("Withdrawals", function () {
    ...
    describe("Transfers", function () {
      it("Should transfer the funds to the owner", async function () {
        ...
        const lockBalanceBefore = await ethers.provider.getBalance(
          this.lock.address
        );

        const tx = await this.lock.withdraw();
        ...
        const lockBalanceAfter = await ethers.provider.getBalance(
          this.lock.address
        );

        // Owner receives locked amount minus gas costs
        expect(ownerBalanceAfter.sub(ownerBalanceBefore).add(gasUsed)).to.equal(
          this.lockedAmount
        );
        // Lock balance should be zero after withdrawal
